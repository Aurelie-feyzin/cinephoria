...
const Movies = () => {
    ...
    const {error, status} = useQuery("movie_in_cinema", () => fetchMovieInCinema(today, lastDay), {
        onSuccess: (data) => {
            ...
            setGenres(_.orderBy(_.uniqBy(newMovies.map((movie: Movie) => movie.genres).flat(), '@id'), 'name'));
        }
    });
    ...
    useEffect(() => {
        const fetchFilteredMovies = () => {
            ...
            if (selectedDay) {
                filtered.map((movie) => console.log(movie.movieShows.find((show) => {
     ...
     <div className="container mx-auto p-6">
         ...
         {selectedMovie && (
             <div className="fixed inset-x-0 bottom-0 bg-white p-4 shadow-lg md:inset-y-0 md:right-0 md:top-0 md:w-1/3">
                 ...
                 <ul>
                     {_.orderBy(selectedMovie.movieShows, ['movieTheater.cinema.name', 'date'])
                         ...
                         .filter((show: MovieShow)=> {
                             let isOk = true;
                             if (selectedCinema) {
                                 isOk = show.movieTheater.cinema.name === selectedCinema;
                             }
                             if (selectedDay) {
                                 isOk = new Date(show.date).getDay() == selectedDay;
                             }
